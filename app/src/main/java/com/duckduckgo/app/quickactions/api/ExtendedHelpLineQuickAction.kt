...
class ExtendedHelpLineQuickAction @Inject constructor(private val context: Context, private val moshi: Moshi) : QuickAction {
    ...
    override fun getQuickActions(query: String): Observable<List<QuickAnswerSuggestion>> {
        ...
        return Observable.just(answer.contacts.map {
            ...
            val mapOfIntents = mapOf(
                "Call Now For Help" to firstIntent
            )

            QuickAnswerSuggestion.MultipleIntentSuggestion(
                it.web,
                it.name,
                mapOfIntents,
                R.drawable.ic_phone,
